...
const PARTICIPANTS_QUERY =
    'SELECT id, type, short_name, full_name, team, family FROM participants order by id';
const PARTICIPANTS_EMAIL_QUERY = 
    'SELECT id, type, short_name, full_name, email, team, family FROM participants order by id';
...
const OCCASIONS_TIME_QUERY = 
    'SELECT o.id as occasion, o.type, o.subtype, o.time as when, r.id as role, r.type as roletype, r.count, a.id as attendance, a.type as attype, a.participant, a.substitute, a.team ' + 
    'FROM occasions o, roles r, attendances a where o.id = r.occasion and r.id = a.role and o.time >= $1 order by o.time, o.id, r.id, a.type, a.id';
const ATTENDANCE_ROLE_QUERY = 
    ...
    'FROM roles r, attendances a WHERE r.id = (SELECT role FROM attendances WHERE id=$1) AND a.role=r.id';
...
const ATTENDANCE_SUBSTITUTE_VALIDATION = 
    "SELECT id FROM attendances WHERE ((participant = $2 AND type != 'declined') OR (substitute=$2 AND id!=$1)) AND role in (SELECT id FROM roles WHERE occasion = " +
        " (SELECT occasion FROM roles WHERE id=(SELECT role FROM attendances WHERE id=$1 )))";
